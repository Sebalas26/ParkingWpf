...
### [2026-09-09 21:35:00] - [BUGFIX / SYNC-TIME / TIMEZONE / 12H-FORMAT] - Corrección de Desfase de Zona Horaria UTC en Píldora de Sincronización y Unificación a Formato de 12 Horas con Indicador AM/PM

- **Autor**: Antigravity AI Assistant & Software Architect
- **💬 Prompt Original del Usuario**:
  > _"bien, valida porque no me esta mostrando la hora ultima en que se sincronizo , no me muestra la hora correcta"_

- **🤖 Resumen Técnico para la IA**:
  1. **Diagnóstico del Desfase de 5 Horas por Tiempo UTC (`SyncEngineService.cs`)**:
     - Se identificó que `SyncEngineService.cs` asignaba `_lastSyncTime = DateTime.UtcNow;` al concluir exitosamente la sincronización o al reconectar.
     - Al estar la terminal en la zona horaria de Colombia (UTC-5), una sincronización realizada a las 08:26 PM (20:26) o 09:26 PM (21:26) almacenaba la hora UTC 01:26 o 02:26 respectivamente.
     - La propiedad `SyncStatusDescription` formateaba con `_lastSyncTime.Value:HH:mm`, imprimiendo directamente la hora UTC `(01:26)` sin convertir a la hora local del terminal y en formato militar 24 horas.
  2. **Persistencia en Hora Local y Conversión Defensiva (`SyncEngineService.cs`)**:
     - Se actualizó la asignación a `_lastSyncTime = DateTime.Now;` en `PerformFullSyncWithProgressAsync` y `SetOnlineStatus`.
     - Se implementó el método `FormatSyncTime(DateTime time)` con conversión defensiva `time.Kind == DateTimeKind.Utc ? time.ToLocalTime() : time` y formateo estándar de 12 horas con indicador de meridiano `hh:mm tt` (`CultureInfo.InvariantCulture`), desplegando limpiamente por ejemplo `Sincronizado (08:26 PM)` o `Sincronizado (09:35 PM)`.
  3. **Unificación de Marcas de Tiempo en Notificaciones Reactivas (`MainShellViewModel.cs`)**:
     - Se actualizaron las notificaciones en caliente de SignalR (actualización de permisos, novedades, cierres/aperturas de caja, horarios y restablecimiento de BD local) para utilizar el formato consistente de 12 horas `hh:mm tt` en `SyncStatusText`.
...
### [2026-09-09 21:20:00] - [UI/UX / SHIFT-CLOSURE / DYNAMIC-PAYMENT-METHODS / ARQUEO-CONSISTENCY] - Métodos de Pago Dinámicos por Sede en Arqueo de Caja y Botón de Cierre en Rojo Claro

- **Autor**: Antigravity AI Assistant & Software Architect
- **💬 Prompt Original del Usuario**:
  > _"Bien, ahora requiero que en esta pantalla el boton de realizar cierre de caja sea un rojo claro, adicional que los valores que esta en el cuadro rojo arriba, sea coherentes y coincidan con los que tengo en la base de datos para mi sede logeada, estoy notandoq que los medios de pago que veo ahi son mockeados y no los quiero"_

- **🤖 Resumen Técnico para la IA**:
  1. **Eliminación de Tarjetas Mockeadas / Hardcodeadas (`ShiftClosureView.xaml`, `ShiftClosureViewModel.cs`)**:
     - Las tarjetas superiores de medios de pago en la pantalla de arqueo ("Entrega de Turno y Arqueo de Caja") ya no son estáticas ni tienen valores quemados ("TARJETAS DÉBITO / CRÉDITO", "TRANSFERENCIAS / QR").
     - Ahora se representan dinámicamente mediante un `ItemsControl` con `UniformGrid Columns="4" Rows="1"` enlazado a la colección observable `PaymentMethodCards`.
  2. **Resolución Dinámica de Medios de Pago de la Sede (`EfShiftService.cs`)**:
     - `GetCurrentShiftSummaryAsync()` ahora consulta en la base de datos local SQLite (`BranchPaymentMethods` y `PaymentMethods`) los medios de pago activos configurados para la sede en sesión (`Sede 236`: Efectivo ID 1, Nequi ID 2, Tarjeta de crédito ID 3).
     - Se auditan los tickets cobrados en el turno vinculando el recaudo según `ticket.PaymentMethodId`.
     - Se eliminó la clasificación errónea por enum `ticket.PaymentMethod == DebitCard || CreditCard` que clasificaba tickets en efectivo (Id: 1) como tarjetas.
     - Ahora los montos son 100% exactos y coherentes: Efectivo = $10.000 (CVCD34 $6.000 + FDFD445 $4.000), Nequi = $2.000 (CCC333 $2.000), Tarjeta de crédito = $0.
     - El balance de caja calcula con precisión: Base ($120.000) + Recaudo Efectivo ($10.000) = Efectivo Esperado en Caja: $130.000.
     - La 4ta tarjeta corresponde al valor consolidado de convenios y descuentos (`Descuentos por Convenios`), manteniendo el layout balanceado de 4 columnas.
  3. **Estilo del Botón de Cierre de Caja (`Colors.xaml`, `ShiftClosureView.xaml`)**:
     - Se definieron los recursos `ColorDangerSoft` (`#EF4444`), `ColorDangerSoftHover` (`#DC2626`), `BrushDangerSoft` y `BrushDangerSoftHover` en `Colors.xaml`.
     - El botón *"Realizar Cierre de Caja"* aplica `Style="{StaticResource DangerButton}" Background="{DynamicResource BrushDangerSoft}"`, brindando el tono rojo claro solicitado y respetando la regla inquebrantable de recursos XAML.
  4. **Convertidor Dinámico de Geometría de Íconos (`IconKeyToGeometryConverter.cs`)**:
     - Creado e instanciado en `App.xaml` para resolver de forma segura las claves de íconos vectoriales (`IconCash`, `IconCreditCard`, `IconQrCode`, `IconTag`) sin riesgos de excepciones XAML.
  5. **Pruebas y Verificación**:
     - `dotnet build ParkingWpf.slnx`: 0 Errores, 0 Advertencias.
     - `dotnet test ParkingWpf.slnx`: **193/193 Superadas (100% éxito, 0 fallos)**.

- **📦 Componentes Modificados**:
  - `Parking/Styles/Colors.xaml`
  - `Parking/Models/ApiModels/ShiftApiModels.cs`
  - `Parking/Core/Converters/IconKeyToGeometryConverter.cs` (Nuevo)
  - `Parking/App.xaml`
...
### [2026-09-09 20:55:00] - [BUGFIX / SYNC / VEHICLE-RATES / OFFLINE-RESILIENCE] - Corrección Integral de Sincronización Multi-Categoría de Tarifas Vehiculares (Visualización de Patineta en Selector y Tarjeta de Tarifas)

- **Autor**: Antigravity AI Assistant & Software Architect
- **💬 Prompt Original del Usuario**:
  > _"Ayudame por que le parametrice a la sede un tipo de vehiculo de patineta y lo visualiza el wpf , valida y veras que llegan mas de los que se muestra, lo extrañp es que cuando parametrizo, el wpf lo detecta y sincroniza pero no lo muestra"_

- **🤖 Resumen Técnico para la IA**:
  1. **Diagnóstico de Colisión de Llave Compuesta en SQLite (`SyncEngineService.cs`)**:
     - Se auditó la respuesta de bootstrap `/api/sync/bootstrap?branchId=1`, verificando que el API enviaba exitosamente 4 tarifas activas para la sede: Carro, Moto, Bicicleta y Patineta.
     - Sin embargo, en la base de datos local SQLite `parkflow_local.db` únicamente persistían 3 tarifas.
     - La causa raíz se identificó en la lógica de sincronización: `var existing = localRates.FirstOrDefault(r => r.RateId == rateId) ?? localRates.FirstOrDefault(r => r.BranchId == targetBranchId && r.VehicleType == vehicleType && r.DayOfWeek == rate.DayOfWeek);`. Al tener tanto Carro como Patineta clasificados en un inicio bajo el mismo enum `VehicleType` (Car o Bicycle), el fallback por tipo de vehículo colisionaba: al iterar Patineta sobreescribía la fila de Carro, y luego al llegar Carro sobreescribía de nuevo sobre la misma fila SQLite, eliminando y ocultando la categoría previa.
  2. **Corrección de Sincronización Quirúrgica por `RateId` (`SyncEngineService.cs`)**:
     - Se eliminó el fallback destructivo por tipo de vehículo en el caso donde `rateId != Guid.Empty`. Cada tarifa se busca y actualiza estrictamente por su identificador único `RateId`.
     - Se corrigió `ratesToDelete`: ahora compara estrictamente contra la lista de IDs entrantes `!incomingRateIds.Contains(r.RateId)`, impidiendo que tarifas válidas de la misma sede sean purgadas de SQLite.
     - Al insertar una nueva tarifa en el contexto EF Core, se agrega inmediatamente a la lista en memoria `localRates.Add(newRate)` para que cualquier iteración posterior reconozca la entidad sin inconsistencias.
  3. **Inferencia Inteligente de Tipos e Íconos Vectoriales (`VehicleTypeHelper.cs`)**:
     - Se robusteció `MapFromApiVehicleType` para evaluar prioritariamente el `fallbackName` cuando se reciben nombres semánticos como `"patin"`, `"scooter"`, `"monopatin"`, mapeándolos automáticamente a `VehicleType.Bicycle` con geometría `IconBike`.
  4. **Preservación de Tarifa Seleccionada en Emisión de Tickets (`CheckInViewModel.cs`)**:
     - En `RegisterAndPrintAsync`, se pasó el valor exacto `customHourlyRate: IsMonthlySubscriber ? 0m : SelectedRate?.HourRate` para que el comprobante de parqueo y el cálculo de cobro posterior utilicen la tarifa de la categoría seleccionada (ej: Patineta $3.800/h) en vez de asumir tarifas predeterminadas.
  5. **Pruebas Unitarias y Certificación**:
     - Se creó la prueba unitaria `SyncEngineService_SyncRates_SavesMultipleCategoriesWithoutCollision` en `OfflineResilienceTests.cs` simulando 4 categorías concurrentes con el mismo BranchId y tipos compartidos.
     - Ejecución de la solución completa con `dotnet test ParkingWpf.slnx`: **193/193 Superadas (100% Éxito, 0 Fallos)**.
     - `dotnet build ParkingWpf.slnx`: **0 Errores, 0 Advertencias**.
...
### [2026-09-09 19:40:00] - [UI/UX / CLEANUP] - Eliminación de Tarifa en Selector Horizontal de Categorías y Eliminación de Operador en Tarjeta Total en Caja

- **Autor**: Antigravity AI Assistant & Software Architect
- **💬 Prompt Original del Usuario**:
  > _"Elimina la tarifa en seleccione la categoria de vehiculo , tambien elimina el operador en total de caja"_

- **🤖 Resumen Técnico para la IA**:
  1. **Depuración de Selector de Categoría (`CheckInView.xaml`)**:
     - Se eliminó el `TextBlock` de tarifa por hora (`$X.XXX/h`) dentro de la tarjeta de selección de categoría de vehículo.
     - Cada elemento de categoría quedó estructurado como una píldora visual limpia y compacta (`Padding="12,8"`) que proyecta únicamente el ícono del vehículo (`IconBadge`) y el nombre de la categoría (`DisplayName`), reduciendo la altura vertical y maximizando la legibilidad.
  2. **Depuración de Tarjeta "TOTAL EN CAJA" (`CheckInView.xaml`)**:
     - Se removió la línea `Operador: {0}` y el texto complementario de turno inactivo debajo del monto de efectivo esperado.
     - La tarjeta ahora se enfoca puramente en el encabezado `"TOTAL EN CAJA"`, su respectivo badge de estado (`TURNO ACTIVO` / `SIN TURNO`) y el monto acumulado en efectivo en tipografía destacada.
  3. **Pruebas Unitarias y Certificación**:
     - Ejecución de la solución completa con `dotnet test ParkingWpf.slnx`: **192/192 Superadas (100% Éxito, 0 Fallos)**.
     - `dotnet build ParkingWpf.slnx`: **0 Errores, 0 Advertencias**.
...
### [2026-09-09 19:25:00] - [UI/UX / FORMATTING] - Formato de Hora Estándar 12 Horas con Indicador AM/PM en Pantalla de Ingreso (Check-In) y Shell Principal

- **Autor**: Antigravity AI Assistant & Software Architect
- **💬 Prompt Original del Usuario**:
  > _"Olvide decirte que la hora no me la pongas en formato 24 horas"_

- **🤖 Resumen Técnico para la IA**:
  1. **Ajuste de Formato de Reloj Digital (`CheckInViewModel.cs`, `MainShellViewModel.cs`)**:
     - Se reemplazó el formato militar de 24 horas (`"HH:mm:ss"`) por el formato de 12 horas estándar con indicador de meridiano (`"hh:mm:ss tt"`, `CultureInfo.InvariantCulture`).
     - Esto proyecta la hora con dos dígitos estables y el sufijo limpio en mayúsculas (`07:24:13 PM`), eliminando caracteres o espacios angostos de localización (`p. m.`) y manteniendo un ancho visual constante sin saltos o recortes en pantalla.
  2. **Pruebas Unitarias y Certificación**:
     - Ejecución de la solución completa con `dotnet test ParkingWpf.slnx`: **192/192 Superadas (100% Éxito, 0 Fallos)**.
     - `dotnet build ParkingWpf.slnx`: **0 Errores, 0 Advertencias**.
...
### [2026-09-09 19:15:00] - [UI/UX / RESPONSIVENESS / CHECK-IN] - Optimización de Pantalla Check-In para Portátiles: Reloj en 2 Líneas Compacto, Selector de Categoría Horizontal (WrapPanel ~5 en fila), Simplificación de Estado de Sincronización, Despliegue de Todas las Tarifas Compactas y Limpieza de Encabezado

- **Autor**: Antigravity AI Assistant & Software Architect
- **💬 Prompt Original del Usuario**:
  > _"Ayudame a organizar esta pantalla del wpf
  - La hora me gusta como se ve, pero para pantalla de portatil no se ve bien y se ve cortada, ajustamela mejor , puede ser que se vea en 2 lineas, pero el componente (el cuadro de por fuera de ellas que sea corto, osea hasta donde va la fecha para no tener espacios al final de la fecha )
  - En seleccione la categoria del vehiculo, podias eliminar esa lista desplegable y dejarmela como unas opciones seleccionables pero a lo horinzal, pero trata que a lo horizontal le quepan como 5 opciones o si no es necesaria dejarlo en 2 lineas
  - En la parte superior, veo que dice api central online, no... eso dejale el texto de sincronizacion y a que hora fue que sincronizo no mas
  - Donde dice tarias seleccionadas, eso dejalo Tarifas , y ahi pones todas las tarifas que recibas de cata tipo de vehiculo que te llegue, para que se mas practico y facil de saber , trata de que no se vean tan grandes porque si son muchas, se desborda la info en pantallas pequeñas 
  - elimina turno"_

- **🤖 Resumen Técnico para la IA**:
  1. **Reloj y Fecha en 2 Líneas Compacto (`CheckInView.xaml`)**:
     - Se rediseñó el panel de hora y fecha para eliminar el desbordamiento en resoluciones de portátiles (1366x768 / 1080p).
     - Se configuró `HorizontalAlignment="Left"` en el contenedor principal `Border` para que su ancho se ajuste milimétricamente al texto de la fecha sin dejar espacios sobrantes a la derecha.
     - Se dispuso en 2 líneas verticales: Línea 1 con la fecha (`CurrentDateString`, 12pt, semibold, slate light) y Línea 2 con la hora digital destacada (`CurrentTimeString`, 18pt, bold/black, gradiente primario).
  2. **Selector Horizontal de Categoría de Vehículo (`CheckInView.xaml`)**:
     - Se eliminó el `ComboBox` desplegable.
     - Se implementó un `ListBox` con `ItemsPanel` basado en `WrapPanel` enlazado a `AvailableRates` y `SelectedItem="{Binding SelectedRate, Mode=TwoWay}"`.
     - Cada elemento es una tarjeta seleccionable (`MinWidth="105"`) que proyecta el ícono del vehículo (`VehicleIconConv`), el nombre del tipo y el costo por hora (`$X.XXX/h`), permitiendo visualizar ~5 opciones en una sola fila u organizarse naturalmente en 2 líneas sin solaparse.
  3. **Simplificación del Estado de Sincronización Superior (`SyncEngineService.cs`, `MainShellViewModel.cs`, `ShiftClosureViewModel.cs`)**:
     - Se eliminó la etiqueta `"API Central Online"` del estado de sincronización.
     - Ahora retorna limpiamente `"Sincronizado (HH:mm)"` o `"Sincronizado"`, conservando la notificación exacta de la hora de sincronización.
     - Se actualizaron las pruebas unitarias de aserción en `OfflineResilienceTests.cs`.
  4. **Tarjetas de Tarifas Compactas y Scrollable (`CheckInView.xaml`)**:
     - Se renombró el bloque `"Tarifa Activa Seleccionada"` a `"Tarifas"`.
     - Se reemplazó la vista unitaria por un listado dinámico (`ItemsControl`) enlazado a `AvailableRates` contenido en un `ScrollViewer` con `MaxHeight="190"`.
     - Despliega cada tipo de vehículo con su ícono y chips compactos de Minuto, Hora y Día Completo sin sobrecargar verticalmente la columna derecha en pantallas reducidas.
  5. **Limpieza del Encabezado de Turno (`CheckInView.xaml`)**:
     - Se eliminó la píldora informativa redundante `"Turno Activo"` del encabezado de la tarjeta *"Últimos Vehículos Ingresados"*.
  6. **Pruebas Unitarias y Certificación**:
     - Se ejecutó la solución completa con `dotnet test ParkingWpf.slnx`: **192/192 Superadas (100% Éxito, 0 Fallos)**.
     - `dotnet build ParkingWpf.slnx`: **0 Errores, 0 Advertencias**.
